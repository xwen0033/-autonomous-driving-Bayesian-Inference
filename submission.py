...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.numRows):
            for col in range(self.belief.numCols):
                dt = math.sqrt((util.colToX(col) - agentX) ** 2 + (util.rowToY(row) - agentY) ** 2)
                pdf = util.pdf(dt, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self):
        ...
        # ### START CODE HERE ###
        posterior = util.Belief(self.belief.numRows, self.belief.numCols, value=0)
        for oldTile, newTile in self.transProb:
            posterior.addProb(newTile[0], newTile[1],
